  ...
  (let ((result-obj #f))
    (page-send-message
     (make-message
      name (jsc->json
            (make-jsc-object
             #f `(("extension" . ,(jsc-context-value "EXTENSION" context))
                  ("args" . ,(scm->jsc args context)))
             context)))
     (lambda (page reply)
...
(define* (default-event-callback event listener arg)
  (g-log "Calling default event callback")
  (jsc-function-call listener arg))
                       ...
                       (cond
                        ...
                        ((eq? #:event type)
                         ...
                         (let* ((callback function)
                                (event-jsc-object (jsc-constructor-call
                           ...
                           (jsc-property-set!
                            ...
                            (make-jsc-function
                             ...
                             #:context context #:number-of-args 3))
...
(define-api "tabs" "Tabs"
  ...
  (list "captureTab" #:method #t 3)
  (list "captureVisibleTab" #:method #t 3)
  ;; TODO: Until message support.
  ;; (list "connect" #:method #t 3)
  (list "create" #:method #t 2)
  (list "detectLanguage" #:method #t 2)
  (list "discard" #:method #t 2)
  (list "duplicate" #:method #t 3)
  (list "executeScript" #:method #t 3)
  (list "get" #:method #t 2)
  (list "getAllInWindow" #:method #t 2)
  (list "getCurrent" #:method #t 1)
  (list "getSelected" #:method #t 2)
  (list "getZoom" #:method #t 2)
  (list "getZoomSettings" #:method #t 2)
  (list "goForward" #:method #t 2)
  (list "goBack" #:method #t 2)
  (list "hide" #:method #t 2)
  (list "highlight" #:method #t 2)
  (list "insertCSS" #:method #t 3)
  (list "move" #:method #t 3)
  (list "moveInSuccession" #:method #t 4)
  (list "print" #:method #t 1)
  (list "printPreview" #:method #t 1)
  ...
  (list "saveAsPDF" #:method #t 2)
  ;; TODO: Until message support.
  ;; (list "sendMessage" #:method #t 4)
  ;; (list "sendRequest" #:method #t 3)
  (list "setZoom" #:method #t 3)
  (list "setZoomSettings" #:method #t 3)
  (list "show" #:method #t 2)
  (list "toggleReaderMode" #:method #t 2)
  (list "update" #:method #t 3)
  ...
  (jsc-context-value-set!
   ...
   (jsc-context-evaluate%
    ...
    hasListener (listener) {
        return !!this._listeners.find(l => l.callback === listener);
    }

    _run (...data) {
        if (this._callback) {
            for (const listener of this._listeners)
                this.callback(this, listener, ...data);
        }
    }
}; ev"
    context)
...
~s"
    ...
    (cond
     ...
     ((string=? name "event")
      ...
      (hash-map->list
       (lambda (name we)
         (g-log "~s extension ~s" name we)
         (map (lambda (context)
                (let ((event (jsc-context-evaluate%
                              (string-append "browser." (jsc-property param-jsc "name"))
                              context)))
                  (when (jsc-object? event)
                    (jsc-object-call-method
                     event "_run"
                     (jsc-property% (jsc-property% param-jsc "args") 0)))))
              (we-jsc-contexts we)))
       *web-extensions*)
      (message-reply message))
     (else
     ...
     ((string=? name "event")
      ...
      (hash-map->list
       (lambda (name we)
         (g-log "~s extension ~s" name we)
         (map (lambda (context)
                (g-log "Trying context ~s for extension ~s" context (we-name we))
                (let ((event (jsc-context-evaluate%
                              (string-append "browser." (jsc-property param-jsc "name"))
                              context)))
                  (when (jsc-object? event)
